...
export default function DriverPage() {
  ...
  useEffect(() => {
    const interval = setInterval(() => tentarSync(), 10000);
    // Sync ao abrir
    setTimeout(() => tentarSync(), 1000);
    return () => clearInterval(interval);
  }, []);
  ...
  async function tentarSync() {
    ...
    try {
      ...
      if (n > 0) {
        setSyncToast(`${n} registro(s) sincronizado(s).`);
    ...
    <div className="min-h-screen flex flex-col bg-[#f5f5f5] text-slate-900 font-sans">
      <header className="h-14 bg-white border-b border-slate-200 px-6 flex items-center justify-between shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <span className="text-sm font-bold text-slate-800">Auris Frota</span>
            <span className="text-[10px] text-slate-400 font-semibold ml-2 hidden sm:inline italic">Quiosque</span>
          </div>
        </div>
        <div className="flex items-center gap-3">
          {pendingSyncCount > 0 && (
            <div className="flex items-center gap-2 px-2 py-0.5 bg-amber-50 text-amber-700 border border-amber-200 rounded-full font-bold">
              <div className="w-1.5 h-1.5 rounded-full bg-amber-500 animate-pulse"></div>
              <span className="text-[10px]">{pendingSyncCount} pendente(s)</span>
            </div>
          )}
          <button
      ...
      <main className="flex-1 flex flex-col">
        <DriverApp onEventoRegistrado={() => tentarSync()} />
      </main>
    </div>
  );
}
